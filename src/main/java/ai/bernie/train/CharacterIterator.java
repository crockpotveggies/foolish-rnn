...
public class CharacterIterator implements DataSetIterator {
  ...
  public CharacterIterator(String textFilePath, Charset textFileEncoding, int miniBatchSize, int exampleLength,
                           String[] validCharacters, Random rng) throws IOException {
  ...
  /** A minimal character set, 1-49 and common punctuation etc */
  public static String[] getMinimalCharacterSet(){
    int[] numbers = IntStream.range(1, 50).toArray();
    String[] numberChars = new String[50];
    for(int i=0; i < numbers.length; i++) {
      numberChars[i] = Integer.toString(numbers[i]);
    }
    numberChars[49] = "\n";
    return numberChars;
  }
